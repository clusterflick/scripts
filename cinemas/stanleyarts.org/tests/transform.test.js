...
// What the retrieve hands over once it has stood down for the closure: the site
// refuses every request, so there are no pages to read events off.
const noPages = { movieListPages: [] };

describe("stanley arts transform with no listings", () => {
  ...
  it("fails when the venue is not expected to be closed", async () => {
    setToday("2026-07-11");
    await expect(transform(noPages, {})).rejects.toThrow(
      "No movies found — page structure may have changed",
    );
  });
  ...
  (closure ? it : it.skip)(
    ...
    async () => {
      ...
      await expect(transform(noPages, {})).resolves.toEqual([]);
