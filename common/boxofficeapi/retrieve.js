...
// Shared by every chain whose site is Webedia's Gatsby build, which proxies
...
async function retrieve({ domain, cinemaId }, { listingPath, cachePrefix }) {
  ...
  const pageData = await dailyCache(`${cachePrefix}-page-data`, async () =>
    fetchJson(`${requestPrefix}page-data/${listingPath}/page-data.json`),
  );
  ...
  for (const hash of pageData.staticQueryHashes) {
    const data = await dailyCache(`${cachePrefix}-${hash}`, async () =>
