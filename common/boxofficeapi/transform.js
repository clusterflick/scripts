...
async function transform(
  ...
  const movies = movieData.reduce((moviesAtThreate, movie) => {
    ...
    // Some chains leave the synopsis off the page data and only carry it on
    // the movie details - Cineworld's page data has none at all.
    const synopsis = movie.synopsis || movieInfo.synopsis;
    ...
    const performances = Object.values(movieListPage[movie.id])
      ...
      .filter((performance, index, all) => {
        // The API occasionally returns duplicate performance entries.
      ...
      .map((performance) => {
        ...
        performance.tags.forEach((tag) => {
          ...
          const tagAccessibility = accessibilityTags[tag.toLowerCase()];
          if (tagAccessibility) {
            accessibility = { ...accessibility, ...tagAccessibility };
            return;
          }
